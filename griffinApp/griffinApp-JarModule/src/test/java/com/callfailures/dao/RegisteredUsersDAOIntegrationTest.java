...
public class RegisteredUsersDAOIntegrationTest {
	...
	public static JavaArchive createTestArchive() {
		return ShrinkWrap.create(JavaArchive.class, "test.jar")
				.addClasses(User.class, UsersDAO.class, UserGenerator.class).addAsManifestResource(
						"arquillian-persistence/persistence.xml", ArchivePaths.create("persistence.xml"));
	}

	@Inject
	UsersDAO registeredUsersDAO;
...
//		   @Test
	public void testGetUserByIdSuccess() {
		...
		user.setUserPassword(userPassword);
		registeredUsersDAO.addUser(user);
		final User queryResult = registeredUsersDAO.getUserByUserId(userId);
		assertEquals(userId, queryResult.getUserId());
	}

//		   @Test
	public void testGetUserByIdFailure() {
		final User queryResult = registeredUsersDAO.getUserByUserId(null);
		assertEquals(userId, queryResult.getUserId());
	}
