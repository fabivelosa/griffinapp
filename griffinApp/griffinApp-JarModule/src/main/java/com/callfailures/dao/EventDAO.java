...
public class EventDAO {
	private static final String FIND_ALL_EVENTS = "SELECT e FROM event e",
			...
			FIND_IMSI_COUNT_FOR_TIMEPERIOD = "SELECT DISTINCT NEW com.callfailures.entity.views.IMSICount(e.imsi, COUNT(e)) "
					+ "FROM event e " + "WHERE (e.dateTime BETWEEN :startTime AND :endTime) " + "GROUP BY e.imsi "
			...
			FIND_CALL_FAILURES_BY_IMSI = "SELECT NEW com.callfailures.entity.views.IMSIEvent(e.imsi,e.eventCause) "
					+ "FROM event e " + "WHERE e.imsi = :imsi " + "GROUP BY e.imsi, e.eventCause",
			FIND_CALL_FAILURES_BY_IMSI_AND_DATE = "SELECT NEW com.callfailures.entity.views.IMSISummary(e.imsi, COUNT(e), SUM(e.duration)) "
					+ "FROM event e " + "WHERE (e.dateTime BETWEEN :startTime AND :endTime) " + "AND e.imsi = :imsi "
					+ "GROUP BY e.imsi",
			FIND_CALL_FAILURES_BY_PHONEMODEL_AND_DATE = "SELECT NEW com.callfailures.entity.views.PhoneModelSummary(e.ueType.model, COUNT(e)) "
					+ "FROM event e " + "WHERE (e.dateTime BETWEEN :startTime AND :endTime) "
					+ "AND e.ueType.model = :model " + "GROUP BY e.ueType.model",
	...
	/**
	 * Queries the Unique EventID, and CauseCode combinations, and their count by
	 * phone model
	 * 
	...
	/**
	 * Queries the IMSISummary object which includes the count of call failures and
	 * total duration in a given period
	 * 
	 * @param imsi
	...
	/**
	 * Queries the PhoneModelSummary object which includes the count of call
	 * failures in a given period
	 * 
	 * @param ueType
	 * @param startTime (inclusive) - the start of the period
	 * @param endTime   (inclusive) - the end of the period
	...
	public PhoneModelSummary findCallFailuresCountByPhoneModelAndDate(final String model, final LocalDateTime startTime,
			final LocalDateTime endTime) {
		final Query query = entityManager.createQuery(FIND_CALL_FAILURES_BY_PHONEMODEL_AND_DATE,
				PhoneModelSummary.class);
	...
	/**
	 * Query Database for all IMSI with failures between Start and End time
	 * submitted
	 * 
	 * @param startTime (inclusive) - the start of the period
	 * @param endTime   (inclusive) - the end of the period
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<Events> findAllIMSIEventsByDate(final String IMSI, final LocalDateTime startTime,
			final LocalDateTime endTime) {
