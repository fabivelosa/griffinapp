...
public interface ValidationService {
	...
	/**
	 * Reads EventID and Cause Code combination from a Row and returns the existing EventCause stored in the database
	 ...
	 * @throws FieldNotValidException if (1) the valid Failure ID is not stored in  the database 
	 * OR (2) either the EventID or Cause Code is invalid (a String, a null or an empty value)
	 */
	EventCause checkExistingEventCause(Row row, int eventColumnNumber, int causeColumnNumber) throws FieldNotValidException;

	
	/**
	 * Reads Failure ID from a Row and returns the existing EventCause stored in the database
	 * @param row - the Row object
	 * @param column -  the 0-index based column number of Failure ID
	 * @return the existing FailureClass object stored in the database
	 * @throws FieldNotValidException if (1) the valid Failure ID is not stored in  the database 
	 * OR (2) either the Failure ID or Cause Code is invalid (a String, a null or an empty value)
	 */
	FailureClass checkExistingFailureClass(Row row, int column) throws FieldNotValidException;

	
	/**
	 * Reads TAC ID from a Row and returns the existing UserEquipment stored in the database
	 * @param row - the Row object
	 * @param column -  the 0-index based column number of TAC ID
	 * @return the existing UserEquipment object stored in the database
	 * @throws FieldNotValidException if (1) the valid TAC ID is not yet stored in  the database 
	 * OR (2) either the TAC ID or Cause Code is invalid (a String, a null or an empty value)
	 */
	UserEquipment checkExistingUserEquipmentType(Row row, int column) throws FieldNotValidException;

	
	/**
	 * Reads MCC ID and MNC ID from a Row and returns the existing MarketOperator stored in the database
	 * @param row - the Row object
	 * @param mCcColumnNumber -  the 0-index based column number of MCC (Mobile Country Code)
	 * @param mNcColumnNumber - the 0-index based column number of MCC (Mobile Network Code)
	 * @return the existing MarketOperator object stored in the database
	 * @throws FieldNotValidException if (1) the valid MCC and MNC combination is not yet stored in  the database 
	 * OR (2) either the TAC ID is invalid (a String, a null or an empty value)
	 */
	MarketOperator checkExistingMarketOperator(Row row, final int mCcColumnNumber, final int mNcColumnNumber) throws FieldNotValidException;
	 ...
	 * @throws FieldNotValidException if the Duration cell is empty, null, or a String
	 */
